...
class Patient:
        def __init__(self, patient_name):
            ...
            """
            self.onto = get_ontology("../../OncologyAid.rdf").load()
            self.missing_info = []
        ...
        def sync_ontology(self):
            ...
            if "SeekImmediateHelp" in str(self.situation):
              self.information.append("Your symptoms seem critical, you should contact your clinic as soon as possible ! Or call : 1177")
              self.informationSend = False
            if "ReferToHuman" in str(self.situation):
              self.information.append("You should contact your clinic to futher evaluation ! Or call : 1177")
              self.informationSend = False
            
            if not self.informationSend:
              res = self.informationSend
              self.informationSend = True
              return res
            else:
              return []
        ...
        def missing_information(self, cancerType, fetchSymptoms, fever):
            patient_instance = self.onto.Patient(self.str_name)
            self.situation = patient_instance.hasSituationAssessment
        ...
        # Check symptoms and fever
            ...
            # Returning questions
            for i in fetchSymptoms:
                if i["word"] == "fever" or i["word"] == "temperature":
                    if self.onto.TempOfPatientJan.hasTemperatureValue == -1:
                        print("what is the temperature?")
                        self.questionToAsk.append("what is the temperature?")
                    else:
                        pass
            ...
            return {"questionToAsk": self.questionToAsk, "information": self.information} 
